...
// 투명도 적용
const toAlpha = (color: string, alpha = 0.15) => {
...
export default function GaugeChart({ status, levels, value }: GaugeChartProps) {
  ...
  const disabledColor = "rgba(229, 234, 243, 0.4)";
  ...
  const backgroundColor = useMemo(() => {
    return levels.map((level, index) =>
      level === status
        ? colors[index]
        : toAlpha(colors[index] ?? disabledColor, 0.15),
    );
  }, [levels, status, colors]);
  ...
  return (
    <div className={styles.wrapper}>
      ...
      />
      {value && (
        <div className={clsx(styles.value, status && styles[status])}>
          {value}
        </div>
      )}
    </div>
